...
class SyntaxUtils:
    ...
    # REGEX -----------------------------------------------------
    def get_variable_name_regex() -> str: return "[A-Za-z_][A-Za-z_\d]*"
    def get_variable_value_regex() -> str: return "[A-Za-z_\d\\" + "\\".join(SyntaxUtils.get_number_operators() + "[]") + "]+"
    ...
    def is_regular_command(command: str) -> bool: # Check if the command contains regular syntax ("PRINT", "NUM", ...)
    ...
    def is_number_update_command(command: str) -> bool: # Check if the command contains a variable update ("A++2")
        ...
        return True if re.match(f"{variableNameRegex}\s*{operatorRegex}\s*{variableValueRegex}", command) else False
    
    def is_variable_assign_command(command: str) -> bool: # Check if the command contains a variable assignment ("A = 5", "MyText = Hello")
        command = SyntaxUtils.clear_execution_amount(command)
        return True if re.findall("[A-Za-z][A-Za-z\d]*\s*=\s*[A-Za-z\d\s\[\]\+\-\*\/]+", command) else False